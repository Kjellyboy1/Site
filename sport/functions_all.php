...
function map() {
	...
	if ($task == "up") {
		...
		mysql_select_db($dbname) or die($dberror);
		$queryC = "SELECT * FROM Vendelen ORDER BY plaats ASC"; 
		$resultC = mysql_query($queryC);
		$c = 10;
		while ($rC = mysql_fetch_array($resultC)) {
			mysql_query("UPDATE Vendelen SET plaats='$c' WHERE id='$rC[id]'");
			$c++;
		}
		$db = mysql_connect($dbhost,$dbuname,$dbpass); 
		mysql_select_db($dbname) or die($dberror);
		$queryU = "SELECT * FROM Vendelen WHERE id='$id'"; 
		$resultU = mysql_query($queryU);
		$rU = mysql_fetch_array($resultU);
		$plaats = $rU[plaats];
		$plaatsU = $plaats+1;
		mysql_query("UPDATE Vendelen SET plaats='$plaats' WHERE plaats='$plaatsU'");
		mysql_query("UPDATE Vendelen SET plaats='$plaatsU' WHERE id='$id'");
		mysql_close($db);
		echo"<script>self.location='index.php';</script>";
  	}
	else if ($task == "down") {
		...
		$db = mysql_connect($dbhost,$dbuname,$dbpass); 
		mysql_select_db($dbname) or die($dberror);
		$queryD = "SELECT * FROM Vendelen WHERE id='$id'"; 
		$resultD = mysql_query($queryD);
		$rD = mysql_fetch_array($resultD);
		$plaats = $rD[plaats];
		$plaatsD = $plaats-1;
		mysql_query("UPDATE Vendelen SET plaats='$plaats' WHERE plaats='$plaatsD'");
		mysql_query("UPDATE Vendelen SET plaats='$plaatsD' WHERE id='$id'");
		mysql_close($db);
		echo"<script>self.location='index.php';</script>";
  	}
	else if ($task == "nieuw") {
		if ($_POST['submit_text']) {
			...
			if ($text == "")  {
				...
				$db = mysql_connect($dbhost,$dbuname,$dbpass); 
				mysql_select_db($dbname) or die($dberror);
				mysql_query("INSERT INTO Vendelen(foto,textfoto,text,plaats,visible) VALUES('$foto','$textfoto','$text','$plaats','$visible')");
				mysql_close($db);
				echo"<script>self.location='index.php';</script>";
			}
		}	
		...
		if ($_POST['submit_text']) {
			...
			if ($text == "")  {
				...
				$db = mysql_connect($dbhost,$dbuname,$dbpass); 
				mysql_select_db($dbname) or die($dberror);
				mysql_query("UPDATE Vendelen SET foto='$foto' WHERE id='$id'");
				mysql_query("UPDATE Vendelen SET textfoto='$textfoto' WHERE id='$id'");
				mysql_query("UPDATE Vendelen SET text='$text' WHERE id='$id'");
				mysql_query("UPDATE Vendelen SET visible='$visible' WHERE id='$id'");
				mysql_close($db);
				echo"<script>self.location='index.php';</script>";
			}
		}	
		...
		$db = mysql_connect($dbhost,$dbuname,$dbpass); 
		mysql_select_db($dbname) or die($dberror);
		$queryH = "SELECT * FROM Vendelen ORDER BY plaats ASC"; 
		...
		while ($i2 <= $aantal) {
			...
			if ($id <> "") {
				$db = mysql_connect($dbhost,$dbuname,$dbpass); 
				mysql_select_db($dbname) or die($dberror);
				mysql_query("DELETE FROM Vendelen WHERE id='$id'");
				$i3++;
			}
			$i2++;
		}
	...
	<?	
		...
		$path = "../$rR[mediafolder]";
  		...
  		$db = mysql_connect($dbhost,$dbuname,$dbpass); 
  		mysql_select_db($dbname) or die($dberror);
		$query = "SELECT * FROM Vendelen ORDER BY plaats DESC"; 
		$shorten = $textchars;
		...
		while ($r = mysql_fetch_array($result)) {
			...
			$db = mysql_connect($dbhost,$dbuname,$dbpass); 
			mysql_select_db($dbname) or die($dberror);
			$queryMax = "SELECT * FROM Vendelen ORDER BY plaats DESC"; 
...
function edit() {
	...
	mysql_select_db($dbname) or die($dberror);
	$id = $_REQUEST['id'];
	$query = mysql_query("SELECT * FROM Vendelen WHERE id='$id'");
